...
| 数据 | 方案 |
|---|---|
| 授权历史（Approval 事件） | Etherscan API；达到分页预算必须返回 partial，禁止把不完整结果显示为安全 |
...
- **写入链必须匹配查询链**：写入前切链并 simulation，禁止依赖钱包当前链的隐式状态。
- **unknown 不等于 safe**：索引截断或 RPC 部分失败必须显示 partial，禁止展示“很干净”。
- **不持久化用户数据**：服务端路由只瞬时处理查询地址，不记录或建立画像；涉及 Etherscan、RPC、CoinGecko 的第三方数据流必须在 README 如实披露。
