...
export const messages = {
  zh: {
    ...
    approvals: {
      ...
      filterLabel: '筛选授权',
      filterAll: '全部',
      filterAttention: '需关注（含待核实）',
      filterUnlimited: '无限 / 全集合授权',
      sortLabel: '排序',
      sortRisk: '风险优先',
      sortExposure: '估算美元暴露金额',
      sortAsset: '资产名称',
      exposureSortNote: '无报价或余额未知的条目排在后面，不代表零风险',
      noFilterResults: '当前筛选下没有授权；可切回“全部”查看。',
      revokeEffect: '撤销不会转走或取回资产；之后使用相关应用时，可能需要重新授权。',
    ...
    decoder: {
      title: '交易解码',
      description: '解读已提交交易或一段调用数据；不预测执行结果',
      network: '交易所在网络',
      inputType: '输入类型',
      txMode: '交易哈希',
      calldataMode: 'calldata',
      txPlaceholder: '0x 开头的 64 位交易哈希',
      calldataPlaceholder: '0x095ea7b3…',
      txHelp: '查询已提交到所选网络的交易；这里不预览待签交易。',
      calldataHelp: '只解读这段调用数据；没有交易上下文时，无法确定发起者和目标合约。',
      invalidTxHash: '请输入 0x 开头、后接 64 位十六进制字符的交易哈希。',
      invalidCalldata: '请输入 0x 开头、至少包含 4 字节函数选择器的有效十六进制数据。',
      txLookupFailed: '读取或解码交易失败；请核对哈希和所选网络，或稍后重试。',
      calldataFailed: '这段 calldata 未能解码；请检查内容是否完整。',
      noCalldata: '这笔交易没有 calldata，无法进行函数解码；可能是普通主链币转账。',
      txResultNote: '以下是已提交交易的静态解码，不代表交易执行成功。',
      calldataResultNote: '以下仅解读粘贴的 calldata；没有交易上下文，不代表实际执行。',
  ...
  en: {
    ...
    approvals: {
      ...
      filterAll: 'All',
      filterAttention: 'Needs attention (including unverified)',
      filterUnlimited: 'Unlimited / entire collection',
      sortLabel: 'Sort by',
      sortRisk: 'Risk first',
      sortExposure: 'Estimated USD exposure',
      sortAsset: 'Asset name',
      exposureSortNote: 'Unpriced or unknown balances appear last, not as zero risk',
      noFilterResults: 'No approvals match this filter. Select All to see every approval.',
      revokeEffect: 'Revoking does not move or recover assets. The app may ask you to approve again later.',
    ...
    decoder: {
      ...
      description: 'Inspect a submitted transaction or call data; execution is not predicted',
      network: 'Transaction network',
      inputType: 'Input type',
      txMode: 'Transaction hash',
      calldataMode: 'calldata',
      txPlaceholder: '0x-prefixed, 64-character transaction hash',
      calldataPlaceholder: '0x095ea7b3…',
      txHelp: 'Looks up a transaction submitted on the selected network. This does not preview a pending signature.',
      calldataHelp: 'Decodes only these bytes. Without transaction context, the sender and target contract are unknown.',
      invalidTxHash: 'Enter a 0x-prefixed transaction hash with 64 hexadecimal characters.',
      invalidCalldata: 'Enter valid 0x-prefixed hexadecimal data with at least a 4-byte function selector.',
      txLookupFailed: 'Could not retrieve or decode this transaction. Check the hash and network, or retry later.',
      calldataFailed: 'Could not decode this calldata. Check that it is complete.',
      noCalldata: 'This transaction has no calldata to decode; it may be a plain native-token transfer.',
      txResultNote: 'Static decoding of a submitted transaction; this does not establish execution success.',
      calldataResultNote: 'Only the pasted calldata was decoded; no transaction context or execution is implied.',
