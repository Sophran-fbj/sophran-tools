...
function decodeCall(
  ...
  // 2) 普通调用：本地已知签名表 / 注入签名表（可能有歧义，逐个尝试）。
...
function buildContainerInfo(
  ...
  switch (definition.kind) {
    ...
    case 'entrypoint-handleOps':
    case 'entrypoint-handleOps-legacy': {
      const version = target ? ENTRYPOINT_ADDRESSES[target] : undefined;
      const fallback = definition.kind === 'entrypoint-handleOps' ? 'v0.7+' : 'v0.6';
      return {
        kind: definition.kind,
        ...
        note: version ? undefined : 'entrypoint-address-unknown',
      };
    }
    case 'multicall-bytes':
      return {
