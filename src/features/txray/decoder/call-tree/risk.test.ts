...
describe('风险规则：permit / Permit2', () => {
  ...
  it('permit2-permit：Permit2 地址 + 官方选择器 → high，evidence 来自嵌套字段', () => {
    const deadline = NOW_SEC + 3600n;
    const result = buildCallTree(
      {
        to: ADDR.permit2,
        data: encodePermit2PermitTransferFrom(deadline),
      },
      { nowMs: NOW_MS },
    );
    const hits = findByCode(result.root, 'permit2-permit');
    expect(hits).toHaveLength(1);
    // permitTransferFrom 没有 spender 字段：被授权方是调用 Permit2 的合约
    expect(hits[0].finding.evidence.spender).toBe('msg.sender');
    // deadline 从嵌套 tuple 中提取（不再是 unknown）
    expect(hits[0].finding.evidence.deadline).toBe(deadline.toString());
  });

  it('permit2-permit（allowance 变体）：spender 从嵌套 tuple 提取', () => {
    const deadline = NOW_SEC + 3600n;
    const result = buildCallTree(
      {
        to: ADDR.permit2,
        data: encodePermit2AllowancePermit(deadline),
      },
      { nowMs: NOW_MS },
    );
    expect(result.root.permit2?.spender?.toLowerCase()).toBe(
      ADDR.spender.toLowerCase(),
    );
    expect(result.root.permit2?.deadline).toBe(deadline.toString());
    const hits = findByCode(result.root, 'permit2-permit');
    expect(hits[0].finding.evidence.spender).toBe(
      '0xE592427A0AEce92De3Edee1F18E0157C05861564',
    );
  ...
  it('Permit2 sigDeadline 已过期 → deadline-suspicious', () => {
    const result = buildCallTree(
      {
        to: ADDR.permit2,
        data: encodePermit2AllowancePermit(NOW_SEC - 120n),
      },
      { nowMs: NOW_MS },
    );
    const hits = findByCode(result.root, 'deadline-suspicious');
    expect(hits).toHaveLength(1);
    expect(hits[0].finding.evidence.reason).toBe('expired');
    expect(hits[0].finding.evidence.param).toBe('deadline');
  });

  it('allowance permit 数据截断时 spender 报 unknown 而非 msg.sender', () => {
    // allowance 变体本应有嵌套 spender；calldata 截断导致解不出来时，
    // 不能声称被授权方是 msg.sender
    const selector = encodePermit2AllowancePermit(NOW_SEC + 3600n).slice(0, 10);
    const result = buildCallTree(
      {
        to: ADDR.permit2,
        data: (selector + '00'.repeat(64)) as `0x${string}`,
      },
      { nowMs: NOW_MS },
    );
    expect(result.root.permit2).toBeNull();
    const hits = findByCode(result.root, 'permit2-permit');
    expect(hits).toHaveLength(1);
    expect(hits[0].finding.evidence.spender).toBe('unknown');
    expect(hits[0].finding.evidence.deadline).toBe('unknown');
  });
...
describe('风险规则：容器与调用语义', () => {
  it('Safe DELEGATECALL → high（CALL 无此 finding）', () => {
    ...
    const hits = findByCode(delegate.root, 'safe-delegatecall');
    expect(hits).toHaveLength(1);
    // evidence 的 target 是 execTransaction 的 to（真正被 delegatecall 的合约），
    // 不是 Safe 自身
    expect(hits[0].finding.evidence.target).toBe(
      '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
    );
    expect(hits[0].finding.evidence.target).not.toBe(ADDR.safeProxy);
