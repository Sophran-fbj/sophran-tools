...
describe('buildCallTree: ERC-4337 EntryPoint', () => {
  ...
  it('handleOps（v0.6 布局）：按形状识别并保留 raw 字段', () => {
    ...
    // v0.7 地址 + v0.6 布局 = 地址与 ABI 版本不匹配：不能声称均匹配，
    // 必须降级为形状识别并明确提示矛盾
    expect(result.root.container?.kind).toBe('entrypoint-handleOps-legacy');
    expect(result.root.container?.identification).toBe('selector-and-abi-shape');
    expect(result.root.container?.version).toBe('v0.6');
    expect(result.root.container?.note).toBe('entrypoint-layout-mismatch');
  ...
  it('handleOps（packed 布局 + v0.6 地址）：同样标记版本不匹配', () => {
    const data = encodeHandleOpsPacked([
      { sender: ADDR.account, callData: encodeTransfer(ADDR.recipient, 1n) },
    ]);
    const result = buildCallTree({ to: '0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789', data });
  ...
  it('handleOps（v0.6 地址 + v0.6 布局）：官方地址与 ABI 匹配', () => {
    const data = encodeHandleOpsLegacy([
      { sender: ADDR.account, callData: encodeTransfer(ADDR.recipient, 1n) },
    ]);
    const result = buildCallTree({ to: '0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789', data });

    expect(result.root.container?.identification).toBe(
      'entrypoint-canonical-address-and-abi',
    );
    expect(result.root.container?.version).toBe('v0.6');
    expect(result.root.container?.note).toBeUndefined();
  });
...
describe('buildCallTree: 稳定性与不变量', () => {
  ...
  it('Permit2 permit：无需签名库即可本地解码并提取嵌套字段', () => {
    const deadline = NOW_SEC + 3600n;
    const result = buildCallTree(
      { to: ADDR.permit2, data: encodePermit2PermitTransferFrom(deadline) },
      { nowMs: FIXED_NOW_MS },
    );
    expect(result.root.decodeStatus).toBe('decoded');
    expect(result.root.functionName).toBe('permitTransferFrom');
    expect(result.root.permit2?.spender).toBeNull();
    expect(result.root.permit2?.deadline).toBe(deadline.toString());
  });

  it('Permit2 allowance permit：spender 从嵌套 tuple 提取', () => {
    const deadline = NOW_SEC + 3600n;
    const result = buildCallTree(
      { to: ADDR.permit2, data: encodePermit2AllowancePermit(deadline) },
      { nowMs: FIXED_NOW_MS },
    );
    expect(result.root.decodeStatus).toBe('decoded');
    expect(result.root.functionName).toBe('permit');
