...
// 调用树展示：可折叠层级 + 问题路径默认展开（高风险 / 未知或不完整 /
// 递归停止）+ 键盘可达 + 双语。
...
export default function CallTreeView({
  ...
  const { t } = useI18n();
  // 默认展开：所有子树存在问题（高风险 / 未知或不完整 / 递归停止 /
  // 显著 warning）的节点及其祖先链，保证"unknown 不等于 safe"不被折叠。
  const [expanded, setExpanded] = useState<Set<string>>(() =>
    collectDefaultExpandedPaths(tree.root),
  );
...
function TreeNodeView({
  ...
  const incompleteCount = inspectSubtree(node).incomplete;
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 py-0.5">
        ...
        {incompleteCount > 0 && (
          <span
            ...
            title={t.decoder.tree.statusLabels.unknown}
          >
            {t.decoder.tree.incompleteBadge(incompleteCount)}
          </span>
        )}
      </div>
